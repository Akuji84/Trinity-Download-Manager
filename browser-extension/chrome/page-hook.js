(function () {
  ...
  window.open = function patchedWindowOpen(url, target, features) {
    ...
    requestCapture(
      ...
      (captured, fallbackToBrowser) => {
        if (!captured && fallbackToBrowser) {
  ...
  function requestCapture(payload, onResult) {
    ...
    const finish = (captured, fallbackToBrowser) => {
      if (settled) {
        return;
      }
      settled = true;
      ...
      onResult(captured, fallbackToBrowser);
    };
    ...
    const resultHandler = (event) => {
      ...
      finish(event.detail?.captured === true, event.detail?.fallbackToBrowser === true);
    };
